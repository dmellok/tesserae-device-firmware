...
static void draw_ble(void)
{
    ...
    int content_h = s_H;
#ifdef TESSERAE_BLE_SETUP_AVAILABLE
    const char *exit_hint = "Press Refresh to exit";
    int exit_s = text_scale_to_fit(exit_hint, base_s, s_W * 5 / 6);
    content_h -= 8 * exit_s + gap;
    draw_text_in(0, s_W, content_h + gap / 2, exit_hint, exit_s, COL_BLK);
#endif
    ...
    if (s_W > s_H) {
        ...
        int instruction_h = split_instruction ? 16 * body_s + gap / 2
                                              : 8 * body_s;
        int logo = (content_h * 2) / 5;
        if (logo > text_area_w * 2 / 3) logo = text_area_w * 2 / 3;
        int text_block = gap + 8 * wordmark_s + gap + 8 * title_s + gap
                         + instruction_h + gap + 8 * passkey_s;
        int max_logo = content_h - text_block - 2 * gap;
        if (logo > max_logo) logo = max_logo;
        if (logo < 1) logo = 1;
        int block = logo + text_block;
        int y = (content_h - block) / 2;
        ...
        if (qn) {
            ...
            int qy = (content_h - total) / 2 + 4 * scale;
