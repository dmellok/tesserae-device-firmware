...
static esp_err_t sht4x_init(void)
{
    ...
    /* Get-or-create: on several boards this bus is shared -- with the GT911
     * touch controller on the E1003, with the BQ27220 fuel gauge on the Sticky
     * -- and whichever driver runs first must be the one that creates it. */
    ...
    /* The helper owns the bus lifetime, so a failed add_device leaves it up for
     * the other drivers on it rather than tearing it down under them. */
