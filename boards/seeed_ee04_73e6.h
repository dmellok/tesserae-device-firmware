/*
 * Board: Seeed Studio XIAO ePaper Display Board EE04 + 7.3" Spectra-6 panel
 *   - MCU:   XIAO ESP32-S3 Plus (ESP32-S3 + PSRAM), native USB (no CH340)
 *   - Panel: 7.3" 800x480 6-colour Spectra-6, single controller, on the
 *     EE04's 50-pin FPC (jumper caps must be in the 50-pin position)
 *
 * The EE04 hosts one single-controller panel on either a 24-pin or a 50-pin
 * FPC; this header is the 50-pin 7.3" Spectra-6 build (see seeed_ee04_75.h
 * for the 24-pin mono variant). Same panel class as the E1002 / PhotoPainter
 * 7.3", so it REUSES the spectra6_spi_single driver unchanged.
 *
 ...
 * checked against Seeed_GFX but not yet run on a real EE04. SPI is pinned at
 * a conservative 10 MHz for bring-up (the E1002 runs this driver at 20 MHz).
 *
...
/* Board model -> default device id "Seeed_EE04_73E6_<mac-suffix>". */
#define TESSERAE_DEVICE_MODEL  "Seeed_EE04_73E6"
#define TESSERAE_BLE_HARDWARE_CODE  7
 ...
 * catalog (hardware/<vendor>/seeed_ee04_73e6.json): "protocol" is the
